    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            for (var i = 0; i < response.length; i++) {
                ...
                if (response[i].sender.userId != $('.msg_head').text()) $(div).addClass("text-success list-group-item msg_b");
                else $(div).addClass("text-primary list-group-item msg_a");
                $('.msg_body').append(div);
            }
...
function getChatters() {
    $.ajax({
        type: 'GET',
        url: 'Controller?action=GetChatters',
        dataType: ' text',
        success: function (response) {
            $('.msg_head').html(response);
        },
        error: function () {
