...
public class PersonRepositoryStub implements PersonRepository {
    ...
    public PersonRepositoryStub() {
        ...
        administrator.setGender("unknown");
        administrator.setStatus("Offline");
        administrator.setAge(42);
        add(administrator);

        Person jan = new Person("jan@ucll.be", "t", "Jan", "Janssens", Role.LID);
        jan.setAge(25);
        jan.setGender("male");
        jan.setStatus("Offline");
        add(jan);

        Person an = new Person("an@ucll.be", "t", "An", "Cornelissen", Role.LID);
        an.setAge(30);
        an.setGender("female");
        an.setStatus("Offline");
        add(an);
        jan.addFriend(an);
        an.addFriend(jan);

		/*Enigste persoon zonder vrienden (testen applicatie)*/
		Person tibo = new Person("r0705828@ucll.be", "t", "Tibo", "De Clercq", Role.LID);
		tibo.setAge(20);
		tibo.setGender("male");
		tibo.setStatus("Offline");
		add(tibo);
